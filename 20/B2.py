...
# S to A to B to C to S
# minimize sum(distance) subject to sum(heights)>=0
# what about directions? any start, any end, but need compatible abc
def getCost(sr,sc,start_dir,er,ec):
    Q = deque([])
    for d in range(4):
        Q.append((0,0, 0, sr, sc, 10000, d))
    while Q:
        d, target, r, c, z, dir_ = heapq.heappop(Q)
        if r==er and c==ec:
            OPTS.append(())
